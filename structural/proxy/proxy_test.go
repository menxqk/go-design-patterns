...
func TestUserListProxy(t *testing.T) {
	...
	t.Run("FindUser - Empty cache", func(t *testing.T) {
		...
		if user.ID != knownIDs[0] {
			t.Error("Returned user id does not match with expected")
		}
	...
	t.Run("FindUser - overflowing the stack", func(t *testing.T) {
		...
		if proxy.LastSearchUsedCache {
			t.Error("The user was not stored on the proxy cache yet")
		}
		...
		if len(proxy.StackCache) != 2 {
			t.Error("After inserting 3 users the cache should not grow above 2")
		}
